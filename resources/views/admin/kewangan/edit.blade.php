...
@section('title', __('protege.kw_edit_title') . ' - ' . $kewangan->id_graduan)
@section('page-title', __('protege.kw_edit_title'))

@section('content')
<div class="max-w-4xl">
    <div class="mb-6">
        <a href="{{ route('admin.kewangan.show', $kewangan) }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
    ...
    <div class="mb-4 inline-flex items-center gap-2 bg-blue-50 text-blue-700 px-3 py-1.5 rounded-lg text-sm">
        <span class="font-mono font-bold">{{ $kewangan->id_graduan }}</span>
        <span class="text-blue-400">|</span>
        <span>{{ $kewangan->bulan }} {{ $kewangan->tahun }}</span>
    </div>

    <form method="POST" action="{{ route('admin.kewangan.update', $kewangan) }}">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select id="id_pelaksana" name="id_pelaksana" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                        ...
                        @foreach($pelaksana as $p)
                            <option value="{{ $p->id_pelaksana }}" {{ old('id_pelaksana', $kewangan->id_pelaksana) == $p->id_pelaksana ? 'selected' : '' }}>{{ $p->nama_syarikat }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="bulan" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                        @foreach(['Januari','Februari','Mac','April','Mei','Jun','Julai','Ogos','September','Oktober','November','Disember'] as $b)
                            <option value="{{ $b }}" {{ old('bulan', $kewangan->bulan) === $b ? 'selected' : '' }}>{{ $b }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="tahun" value="{{ old('tahun', $kewangan->tahun) }}" required min="2020" max="2030" step="1" inputmode="numeric" data-numeric="integer"
                ...
                <div>
                    ...
                    <input type="date" name="tarikh_mula_kerja" value="{{ old('tarikh_mula_kerja', $kewangan->tarikh_mula_kerja?->format('Y-m-d')) }}"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('protege.kw_tarikh_akhir') }}</label>
                    <input type="date" name="tarikh_akhir_kerja" value="{{ old('tarikh_akhir_kerja', $kewangan->tarikh_akhir_kerja?->format('Y-m-d')) }}"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('protege.kw_hari_bekerja') }} <span class="text-red-500">*</span></label>
                    <input type="number" name="hari_bekerja_sebenar" value="{{ old('hari_bekerja_sebenar', $kewangan->hari_bekerja_sebenar) }}" required min="0" step="1" inputmode="numeric" data-numeric="integer"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="hari_dalam_bulan" value="{{ old('hari_dalam_bulan', $kewangan->hari_dalam_bulan) }}" required min="1" step="1" inputmode="numeric" data-numeric="integer"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('protege.kw_elaun_penuh') }} <span class="text-red-500">*</span></label>
                    <input type="number" name="elaun_penuh" value="{{ old('elaun_penuh', $kewangan->elaun_penuh) }}" required min="0" step="0.01" inputmode="decimal"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>
                ...
                <div>
                    ...
                    <select name="status_bayaran" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                        @foreach(['Selesai' => __('protege.bayaran_selesai'), 'Dalam Proses' => __('protege.bayaran_dalam_proses'), 'Lewat' => __('protege.bayaran_lewat')] as $val => $label)
                            <option value="{{ $val }}" {{ old('status_bayaran', $kewangan->status_bayaran) === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tarikh_bayar" value="{{ old('tarikh_bayar', $kewangan->tarikh_bayar?->format('Y-m-d')) }}"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('protege.kw_tarikh_jangka') }}</label>
                    <input type="date" name="tarikh_jangka_bayar" value="{{ old('tarikh_jangka_bayar', $kewangan->tarikh_jangka_bayar?->format('Y-m-d')) }}"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="catatan" rows="3"
                              class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">{{ old('catatan', $kewangan->catatan) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="mt-5 flex items-center gap-3">
            ...
            <a href="{{ route('admin.kewangan.show', $kewangan) }}"
...
function graduanSelector() {
    ...
    const editId = '{{ old("id_graduan", $kewangan->id_graduan ?? "") }}';

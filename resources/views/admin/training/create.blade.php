...
@section('title', __('protege.trn_add_record'))
@section('page-title', __('protege.trn_add_record'))
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="jenis_training" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F] @error('jenis_training') border-red-400 @enderror">
                        ...
                        @foreach(['Soft Skills' => __('protege.trn_soft_skills'), 'Technical' => __('protege.trn_technical'), 'Safety' => __('protege.trn_safety'), 'Other' => __('protege.trn_other')] as $value => $label)
                            <option value="{{ $value }}" {{ old('jenis_training') === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="tajuk_training" value="{{ old('tajuk_training') }}" required
                ...
                <div>
                    ...
                    <select name="sesi" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F] @error('sesi') border-red-400 @enderror">
                        ...
                        <option value="Session 1" {{ old('sesi') === 'Session 1' ? 'selected' : '' }}>Session 1</option>
                        <option value="Session 2" {{ old('sesi') === 'Session 2' ? 'selected' : '' }}>Session 2</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="tarikh_training" value="{{ old('tarikh_training') }}" required
                ...
                <div>
                    ...
                    <input type="number" name="durasi_jam" value="{{ old('durasi_jam') }}" required min="1" step="1" inputmode="numeric" data-numeric="integer"
                ...
                <div>
                    ...
                    <select name="trainer_type" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F] @error('trainer_type') border-red-400 @enderror">
                        <option value="">{{ __('protege.select_type') }}</option>
                        <option value="Internal" {{ old('trainer_type') === 'Internal' ? 'selected' : '' }}>{{ __('protege.trn_internal') }}</option>
                        <option value="External" {{ old('trainer_type') === 'External' ? 'selected' : '' }}>{{ __('protege.trn_external') }}</option>
                    </select>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" name="jumlah_dijemput" value="{{ old('jumlah_dijemput') }}" min="0" step="1" inputmode="numeric" data-numeric="integer"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('protege.trn_hadir') }}</label>
                    <input type="number" name="jumlah_hadir" value="{{ old('jumlah_hadir') }}" min="0" step="1" inputmode="numeric" data-numeric="integer"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="topik_covered" rows="3"
                              class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">{{ old('topik_covered') }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('protege.trn_pre_assessment') }}</label>
                    <input type="number" name="pre_assessment_avg" value="{{ old('pre_assessment_avg') }}" min="0" max="10" step="0.1" inputmode="decimal"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('protege.trn_post_assessment') }}</label>
                    <input type="number" name="post_assessment_avg" value="{{ old('post_assessment_avg') }}" min="0" max="10" step="0.1" inputmode="decimal"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('protege.trn_kepuasan') }} (0-10)</label>
                    <input type="number" name="skor_kepuasan" value="{{ old('skor_kepuasan') }}" min="0" max="10" step="0.1" inputmode="decimal"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" name="budget_allocated" value="{{ old('budget_allocated') }}" min="0" step="0.01" inputmode="decimal"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('protege.trn_budget_spent') }}</label>
                    <input type="number" name="budget_spent" value="{{ old('budget_spent') }}" min="0" step="0.01" inputmode="decimal"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>

...
@section('title', __('protege.kh_edit_title') . ' - ' . $kehadiran->id_graduan)
@section('page-title', __('protege.kh_edit_title'))

@section('content')
<div class="max-w-4xl">
    <div class="mb-6">
        <a href="{{ route('admin.kehadiran.show', $kehadiran) }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
    ...
    <div class="mb-4 inline-flex items-center gap-2 bg-blue-50 text-blue-700 px-3 py-1.5 rounded-lg text-sm">
        <span class="font-mono font-bold">{{ $kehadiran->id_graduan }}</span>
        <span class="text-blue-400">|</span>
        <span>{{ $kehadiran->bulan }} {{ $kehadiran->tahun }}</span>
    </div>

    <form method="POST" action="{{ route('admin.kehadiran.update', $kehadiran) }}">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select id="id_graduan" name="id_graduan" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F] @error('id_graduan') border-red-400 @enderror">
                        ...
                        @foreach($talents as $t)
                            <option value="{{ $t->id_graduan }}" data-full-name="{{ $t->full_name }}" data-pelaksana-id="{{ $t->id_pelaksana }}" data-syarikat-id="{{ $t->id_syarikat_penempatan }}" data-syarikat-name="{{ $t->syarikatPenempatan?->nama_syarikat }}" {{ old('id_graduan', $kehadiran->id_graduan) == $t->id_graduan ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="id_syarikat" name="id_syarikat" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                        ...
                        @foreach($penempatan as $s)
                            <option value="{{ $s->id_syarikat }}" {{ old('id_syarikat', $kehadiran->id_syarikat) == $s->id_syarikat ? 'selected' : '' }}>{{ $s->nama_syarikat }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="id_pelaksana" name="id_pelaksana" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                        ...
                        @foreach($pelaksana as $p)
                            <option value="{{ $p->id_pelaksana }}" {{ old('id_pelaksana', $kehadiran->id_pelaksana) == $p->id_pelaksana ? 'selected' : '' }}>{{ $p->nama_syarikat }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="bulan" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                        @foreach(['Januari','Februari','Mac','April','Mei','Jun','Julai','Ogos','September','Oktober','November','Disember'] as $b)
                            <option value="{{ $b }}" {{ old('bulan', $kehadiran->bulan) === $b ? 'selected' : '' }}>{{ $b }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="tahun" value="{{ old('tahun', $kehadiran->tahun) }}" required min="2020" max="2030" step="1" inputmode="numeric" data-numeric="integer"
                ...
                <div>
                    ...
                    <input type="number" name="hari_hadir" value="{{ old('hari_hadir', $kehadiran->hari_hadir) }}" required min="0" step="1" inputmode="numeric" data-numeric="integer"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('protege.kh_hari_bekerja') }} <span class="text-red-500">*</span></label>
                    <input type="number" name="hari_bekerja" value="{{ old('hari_bekerja', $kehadiran->hari_bekerja) }}" required min="1" step="1" inputmode="numeric" data-numeric="integer"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="skor_prestasi" value="{{ old('skor_prestasi', $kehadiran->skor_prestasi) }}" required min="1" max="10" step="1" inputmode="numeric" data-numeric="integer"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                </div>
                ...
                <div>
                    ...
                    <select name="status_logbook" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">
                        @foreach(['Dikemukakan' => __('protege.lb_dikemukakan'), 'Lewat' => __('protege.lb_lewat'), 'Belum Dikemukakan' => __('protege.lb_belum')] as $val => $label)
                            <option value="{{ $val }}" {{ old('status_logbook', $kehadiran->status_logbook) === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="komen_mentor" rows="3"
                              class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]">{{ old('komen_mentor', $kehadiran->komen_mentor) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="mt-5 flex items-center gap-3">
            ...
            <a href="{{ route('admin.kehadiran.show', $kehadiran) }}"

...
<section>
    ...
    <p>{{ __('guide.applications_overview') }}</p>
</section>
...
<section>
    ...
    <div class="bg-gray-50 rounded-lg p-4">
        <p>{{ __('guide.applications_access') }}</p>
    </div>
</section>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        @foreach(range(1, 3) as $i)
            <div class="flex items-start gap-2 bg-blue-50 rounded-lg p-3">
                ...
                <span>{{ __("guide.applications_feat{$i}") }}</span>
            </div>
        @endforeach
    </div>
</section>
...
<section>
    <h2 class="text-lg font-semibold text-gray-800 mb-3 flex items-center gap-2">
        ...
        {{ __('guide.applications_statuses_title') }}
    </h2>
    <div class="overflow-x-auto">
        <table class="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
            ...
            <tbody class="divide-y divide-gray-100">
                @foreach([
                    ['Applied', 'guide.applications_status_applied'],
                    ['Shortlisted', 'guide.applications_status_shortlisted'],
                    ['Approved', 'guide.applications_status_approved'],
                    ['Inactive', 'guide.applications_status_inactive'],
                ] as [$status, $descKey])
                    <tr>
...
<section>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-2">
        @foreach(range(1, 2) as $i)
            <p class="flex items-start gap-2"><span class="text-amber-500 font-bold mt-0.5">!</span>{{ __("guide.applications_tip{$i}") }}</p>
        @endforeach
    </div>
</section>
@endsection

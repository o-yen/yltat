...
<section>
    ...
    <p>{{ __('guide.kehadiran_overview') }}</p>
</section>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
            ...
            <tbody class="divide-y divide-gray-100">
                @foreach([
                    ['Dikemukakan', 'guide.kehadiran_status_dikemukakan'],
                    ['Lewat', 'guide.kehadiran_status_lewat'],
                    ['Belum Dikemukakan', 'guide.kehadiran_status_belum'],
                ] as [$status, $descKey])
                    <tr>
...
<section>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-2">
        @foreach(range(1, 2) as $i)
            <p class="flex items-start gap-2"><span class="text-amber-500 font-bold mt-0.5">!</span>{{ __("guide.kehadiran_tip{$i}") }}</p>
        @endforeach
    </div>
</section>
@endsection

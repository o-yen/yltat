...
<section>
    <h2 class="text-lg font-semibold text-gray-800 mb-3 flex items-center gap-2">
        ...
        {{ __('howto.manage_penempatan_step1_title') }}
    </h2>
    <div class="bg-gray-50 rounded-lg p-4">
        <p>{{ __('howto.manage_penempatan_step1_desc') }}</p>
    </div>
</section>

{{-- Section 2: Track Training --}}
<section>
    <h2 class="text-lg font-semibold text-gray-800 mb-3 flex items-center gap-2">
        <span class="w-7 h-7 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center text-xs font-bold">2</span>
        {{ __('howto.manage_penempatan_step2_title') }}
    </h2>
    <div class="bg-gray-50 rounded-lg p-4">
        <p>{{ __('howto.manage_penempatan_step2_desc') }}</p>
    </div>
</section>

{{-- Section 3: Monitor Compliance --}}
<section>
    <h2 class="text-lg font-semibold text-gray-800 mb-3 flex items-center gap-2">
        <span class="w-7 h-7 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center text-xs font-bold">3</span>
        {{ __('howto.manage_penempatan_step3_title') }}
    </h2>
    <div class="bg-gray-50 rounded-lg p-4">
        <p>{{ __('howto.manage_penempatan_step3_desc') }}</p>
    </div>
</section>

{{-- Warning --}}
<div class="bg-amber-50 border border-amber-200 rounded-lg p-4">
    <p class="flex items-start gap-2"><span class="text-amber-500 font-bold">!</span> {{ __('howto.manage_penempatan_warn') }}</p>
</div>
